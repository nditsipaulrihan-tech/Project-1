...
mysqli_stmt_bind_param($stmt_fetch, "i", $draft_id);
mysqli_stmt_execute($stmt_fetch);
$res = mysqli_stmt_get_result($stmt_fetch);

if ($draft = mysqli_fetch_assoc($res)) {
    $title = $draft['title'];
    $desc = $draft['post_desc'];
    $p_i = $draft['paragraph_i'];
    $p_ii = $draft['paragraph_ii'];
    $p_iii = $draft['paragraph_iii'];
    $p_note = $draft['post_note'];
    $old_img = $draft['img_url'];
} else {
    header("Location: draft.php");
    exit();
}
...
if (isset($_POST['update_draft'])) {
    ...
    if (empty($new_title) || empty($new_desc) || empty($new_p_i)) {
        ...
        if (empty($error)) {
            if ($publish_now) {
                // Publish directly to 'post' table
                $stmt_ins = mysqli_prepare($conn, "INSERT INTO post (title, post_desc, paragraph_i, paragraph_ii, paragraph_iii, post_note, img_url) VALUES (?, ?, ?, ?, ?, ?, ?)");
                ...
                if (mysqli_stmt_execute($stmt_ins)) {
                    ...
                    mysqli_stmt_bind_param($stmt_del, "i", $draft_id);
                ...
                // Save changes to 'draft' table
                $stmt_up = mysqli_prepare($conn, "UPDATE draft SET title=?, post_desc=?, paragraph_i=?, paragraph_ii=?, paragraph_iii=?, post_note=?, img_url=? WHERE id=?");
                mysqli_stmt_bind_param($stmt_up, "sssssssi", $new_title, $new_desc, $new_p_i, $new_p_ii, $new_p_iii, $new_p_note, $new_img, $draft_id);
                
                if (mysqli_stmt_execute($stmt_up)) {
                    header("Location: draft_preview.php?id=" . $draft_id);
                    exit();
                } else {
...
<head>
  ...
  <title>Edit Draft - <?php echo htmlspecialchars($title); ?></title>
</head>
<body>
    ...
    <main>
      <div class="edit-card">
        <div class="edit-header">
          <a href="./draft_preview.php?id=<?php echo $draft_id; ?>" class="back-btn"><i class="fa-solid fa-arrow-left"></i> Back to Draft Preview</a>
          <span class="badge-status badge-drafted">Editing Draft</span>
        </div>
        ...
        <form action="" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group checkbox-group">
            <label><input type="checkbox" name="publish_now"> <b>Publish Immediately</b> (Move directly to published post list)</label>
          </div>

          <button type="submit" name="update_draft" class="save-btn"><i class="fa-solid fa-floppy-disk"></i> Save Draft</button>
        </form>
      </div>
    </main>
