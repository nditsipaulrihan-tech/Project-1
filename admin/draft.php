...
if (!empty($search)) {
    ...
    $sql = "SELECT * FROM draft WHERE title LIKE '%{$safe_search}%' OR post_desc LIKE '%{$safe_search}%' ORDER BY id DESC;";
} else {
  ...
  <section>
    ...
    <header class="nav">
      ...
      <nav>
        ...
        <a href="#"><i class="fa-solid fa-file-pen"></i> Draft</a>
        ...
        if ($post && mysqli_num_rows($post) > 0) {
            foreach ($post as $p) { 
                ...
                <div class='post'>
                  ...
                  <span class='pph' style='background:brown'>Draft</span>
                  <div class='bloga'>
                    <div class='desc'>
                      <p>" . htmlspecialchars($p['post_desc']) . "</p>
                      <small><i class='fa-regular fa-clock'></i> " . htmlspecialchars($p['drafted']) . "</small>
                    </div>
                    <div class='btn'>
                      <a href='draft_preview.php?id={$p['id']}'><button type='button'>View</button></a>
                    </div>
                  </div>
                </div>";
            }
